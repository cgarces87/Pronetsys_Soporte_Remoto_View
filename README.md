...
```
mkdir -p /var/www/pronetsys
wget -q https://raw.githubusercontent.com/cgarces87/Pronetsys_Soporte_Remoto_View/master/docker-compose/docker-compose.yml
docker-compose up -d
```
...
The Pronetsys code does not parse or handle these values. It is done internally by ASP.NET Core's built-in middleware. If the values are not appearing as expected, it is because the headers were missing, didn't contain the correct values, were not in the correct format, or didn't come through a chain of known proxies (see below).

To avoid injection attacks, ASP.NET Core defaults to only accepting forwarded headers from loopback addresses. Pronetsys will also add the Docker gateway IP (172.28.0.1) defined in the docker-compose file. If you are using a non-default configuration, you must add all firewall and reverse proxy addresses to the `KnownProxies` array in the Server Config.
...
- Data for Pronetsys will be saved in the container under `/app/AppData`, which will be mounted to `/var/www/pronetsys/` on your Docker host.
- This folder will contain Pronetsys.db and logs generated by the server.
  - These files will persist through teardown and setup of new Pronetsys containers.
  - If upgrading from a non-Docker version of Pronetsys, overwrite the DB file from your previous installation.
...
The following steps will configure your Windows 11 machine for building the Pronetsys server and clients.
...
- Clone the git repository: `git clone https://github.com/cgarces87/Pronetsys_Soporte_Remoto_View --recurse`
...
- KnownProxies: If your reverse proxy is on a different machine and is forwarding requests to the Pronetsys server, you will need to add the IP of the reverse proxy server to this array.
...
- UseHttpLogging: Enables logging for all HTTP requests. Also enables additional log entries in `ClientDownloadsController` regarding the effective scheme, host, and remote IP address as a result of processing forwarded headers.
  - You must explicitly set a log level for `Microsoft.AspNetCore.HttpLogging.HttpLoggingMiddleware` for this to work. See the [appsettings.json](https://github.com/cgarces87/Pronetsys_Soporte_Remoto_View/blob/master/Server/appsettings.json) for an example.

## Changing the Database

By default, Pronetsys uses a SQLite database. When first run, it creates a file as specified for the SQLite connection string in appsettings.json.

You can change database by changing `DBProvider` in `ApplicationOptions` to `SQLServer` or `PostgreSQL`.

## Logging

- On clients, logs are kept in `%ProgramData%\Pronetsys\Logs`
- Within the server container, logs will be written to `/app/AppData/logs`, which (by default) will be mounted to `/var/www/pronetsys/Logs` on the host.
...
Pronetsys has three roles, and platform support differs per role. It's important
...
- **iOS / iPadOS cannot be remotely controlled.** Apple provides no public API for
  ...
  across apps; this is an OS-level restriction, not a Pronetsys limitation. An
...
A shortcut to this page is placed in the `\Program Files\Pronetsys\` folder. You can copy it anywhere you like. You can also have it copied to the desktop automatically by using the `-supportshortcut` switch on the installer.
...
Pronetsys has a basic API, which can be browsed at `https://{your_server_url}/swagger`. Most endpoints require authentication via an API access token, which can be created by going to Account - API Access.

When accessing the API from the browser on another website, you'll need to set up CORS in appsettings by adding the website origin URL to the TrustedCorsOrigins array. If you're not familiar with how CORS works, I recommend reading up on it before proceeding. For example, if I wanted to create a login form on https://exmaple.com that logged into the Pronetsys API, I'd need to add "https://example.com" to the TrustedCorsOrigins.
...
The Alerts API gives you the ability to add monitoring and alerting functionality to your device endpoints. This feature is intended to add basic RMM-type functionality without diverging too far from Pronetsys's primary purpose.

Alerts can be set up to show a notification on the Pronetsys website, send an email, and/or perform a separate API request.
