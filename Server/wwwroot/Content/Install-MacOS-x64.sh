...
ETag=$(curl --head $HostName/Content/Pronetsys-MacOS-x64.zip | grep -i "etag" | cut -d' ' -f 2)
...
if [ -z "$UpdatePackagePath" ]; then
    ...
    curl $HostName/Content/Pronetsys-MacOS-x64.zip --output $InstallDir/Pronetsys-MacOS-x64.zip
else
    echo  "Copying install files..." >> /tmp/Remotely_Install.log
    cp "$UpdatePackagePath" $InstallDir/Pronetsys-MacOS-x64.zip
    rm -f "$UpdatePackagePath"
fi

unzip -o $InstallDir/Pronetsys-MacOS-x64.zip -d $InstallDir
rm -f $InstallDir/Pronetsys-MacOS-x64.zip
...
curl --head $HostName/Content/Pronetsys-MacOS-x64.zip | grep -i "etag" | cut -d' ' -f 2 > $InstallDir/etag.txt
